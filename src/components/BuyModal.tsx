"use client";

import { useState } from "react";
import useSWR from "swr";
import { useRouter } from "next/navigation";

type MarketType = {
  market: string;
  koreanName: string;
  englishName: string;
};

const fetcher = (url: string) => fetch(url).then((r) => r.json());

export function BuyModal({
  market,
  onClose,
  isLoggedIn,
}: {
  market: MarketType;
  onClose: () => void;
  isLoggedIn: boolean;
}) {
  const router = useRouter();
  const [volume, setVolume] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const { data: tickers = [] } = useSWR(
    `/api/ticker?markets=${market.market}`,
    fetcher,
    { refreshInterval: 2000 }
  );
  const ticker = tickers[0];
  const currentPrice = ticker?.trade_price ?? 0;
  const orderAmount = volume ? parseFloat(volume) * currentPrice : 0;
  const fee = orderAmount * 0.01;
  const total = orderAmount + fee;

  async function handleBuy(e: React.FormEvent) {
    e.preventDefault();
    if (!isLoggedIn) {
      router.push(`/login?callbackUrl=/`);
      return;
    }
    if (!volume || parseFloat(volume) <= 0) {
      setError("수량을 입력해주세요.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      const res = await fetch("/api/trade/buy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ market: market.market, volume: parseFloat(volume) }),
      });
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || "매수에 실패했습니다.");
        return;
      }
      onClose();
      router.refresh();
    } finally {
      setLoading(false);
    }
  }

  return (
    <div
      className="fixed inset-0 bg-black/60 flex items-center justify-center z-50"
      onClick={onClose}
    >
      <div
        className="bg-[#161b22] border border-[#30363d] rounded-lg w-full max-w-md mx-4 p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-white">
            {market.koreanName} 매수
          </h3>
          <button
            onClick={onClose}
            className="text-gray-400 hover:text-white text-xl"
          >
            ×
          </button>
        </div>

        <div className="mb-4 p-3 bg-[#0d1117] rounded text-sm">
          <span className="text-gray-400">현재가 </span>
          <span className="text-white font-medium">
            {currentPrice.toLocaleString()}원
          </span>
        </div>

        <form onSubmit={handleBuy}>
          <div className="mb-4">
            <label className="block text-sm text-gray-400 mb-1">수량</label>
            <input
              type="number"
              step="any"
              min="0"
              value={volume}
              onChange={(e) => setVolume(e.target.value)}
              placeholder="0"
              className="w-full px-3 py-2 bg-[#0d1117] border border-[#30363d] rounded text-white focus:outline-none focus:border-[#00c853]"
            />
          </div>
          {orderAmount > 0 && (
            <div className="mb-4 text-sm text-gray-400 space-y-0.5">
              <div>
                주문금액:{" "}
                <span className="text-white">{orderAmount.toLocaleString()}원</span>
              </div>
              <div>
                수수료 (1%):{" "}
                <span className="text-white">+{fee.toLocaleString()}원</span>
              </div>
              <div>
                결제금액:{" "}
                <span className="text-[#00c853] font-medium">
                  {total.toLocaleString()}원
                </span>
              </div>
            </div>
          )}
          {error && (
            <div className="mb-4 p-2 bg-red-500/20 text-red-400 rounded text-sm">
              {error}
            </div>
          )}
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2 bg-[#00c853] hover:bg-[#00a843] text-black font-semibold rounded disabled:opacity-50"
          >
            {isLoggedIn
              ? loading
                ? "매수 중..."
                : "매수"
              : "로그인 후 매수"}
          </button>
        </form>
      </div>
    </div>
  );
}
